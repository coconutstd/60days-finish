  ...
  class GameData {
    ...
    setCards(cardlist) {
      ...
      for (let index = 0; index < cardlist.length; index++) {
        ...
        while (randomCardList[random] !== undefined) {
          random = Math.floor(Math.random() * 100);
        }
        randomCardList[random] = cardlist[index];
      }
      return randomCardList.filter((item) => item !== undefined);
    }
    ...
    isEnd() {
      // 4번 문제
      if (this.scores === this.total) return "win";
      if (this.limit <= this.moves) return "defeat";
    }
  }
